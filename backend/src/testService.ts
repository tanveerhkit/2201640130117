...
async function run() {
  ...
  try {
    ...
    const { shortLink, short } = await shortenerService.create(
      { url: "https://example.com/long/page", validity: 30, shortcode: "abcd1" },
      origin
    );
    results.push(`CREATE custom -> ${shortLink} (expires ${short.expiresAt})`);

    // 2) Stats before click
    let s1 = await shortenerService.stats("abcd1");
    results.push(`STATS before: clicks=${s1.totalClicks}`);

    // 3) Simulate redirect (records click)
    await shortenerService.resolveAndRecord("abcd1", "http://referrer.com/page", "192.168.1.10");
    let s2 = await shortenerService.stats("abcd1");
    ...
    // 4) Collision check
    try {
      await shortenerService.create({ url: "https://example.com", shortcode: "abcd1" }, origin);
    ...
    // 5) Invalid URL
    try {
      await shortenerService.create({ url: "notaurl" as any }, origin);
    ...
    // 6) Auto-generate
    const auto = await shortenerService.create({ url: "https://example.org/" }, origin);
